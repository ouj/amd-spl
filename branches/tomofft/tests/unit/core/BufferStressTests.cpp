...
TEST_F(BufferStressTests, BufferReadWriteStressTest2)
{
    ...
    info.maxResource2DHeight = 8192;
    info.maxResource2DWidth = 4096;
    ...
    for (unsigned int i = 32; i < cpuBuf.size(); i *= 2)
    {
        ...
        if (buf1D)
        {
            timer.Reset();
            timer.Start();
            ASSERT_TRUE(buf1D->Buffer::readData(&cpuBuf[0], static_cast<unsigned long>(i)));
            timer.Stop();
            printf("Read %d float4 time: %lf\n", i, timer.GetElapsedTime());

            timer.Reset();
            timer.Start();
            ASSERT_TRUE(buf1D->Buffer::writeData(&cpuBuf[0], static_cast<unsigned long>(i)));
            timer.Stop();
...
TEST_F(BufferStressTests, BufferReadWriteStressTest1)
{
    ...
    CPerfCounter timer;
    vector<float4> cpuBuf(info.maxResource2DWidth * info.maxResource2DHeight);
    vector<float4> result(info.maxResource2DWidth * info.maxResource2DHeight);
    util::initializeBuffer(cpuBuf, info.maxResource2DWidth, info.maxResource2DHeight, 1000, util::POS);
    ...
    for (unsigned int i = 32; i < cpuBuf.size(); i *= 2)
    {
        ...
        if (buf1D)
        {
            ...
            timer.Reset();
            timer.Start();
            ASSERT_TRUE(buf1D->Buffer::writeData(&result[0], static_cast<unsigned long>(i)));
            timer.Stop();
            printf("Write %d float4 time: %lf\n", i, timer.GetElapsedTime());
...
TEST_F(BufferStressTests, LocalBufferReadWriteStressTest)
{
    Device* device = _deviceMgr->getDefaultDevice();
    if(!device)
        return;
