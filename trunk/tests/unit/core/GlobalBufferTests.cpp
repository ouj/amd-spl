...
TEST_F(GlobalBufferTests, GlobalBufferTypeCastTest)
{
    ...
    GlobalBuffer *globalBuf = 
        dynamic_cast<GlobalBuffer*>(buf);
    ASSERT_TRUE(globalBuf != NULL);

    _bufMgr->destroyBuffer(globalBuf);
}
...
TEST_F(GlobalBufferTests, GlobalBufferGetPropertiesTest)
{
    ...
    Buffer *buf1D = 
        RuntimeTestFixture::_bufMgr->createGlobalBuffer(device, CAL_FORMAT_FLOAT_2, 1024);
    if (buf1D)
    {
    ...
    Buffer *buf2D = 
        RuntimeTestFixture::_bufMgr->createGlobalBuffer(device, CAL_FORMAT_INT_1, 1024, 1024);
    if (buf2D)
    {
...
TEST_F(GlobalBufferTests, GlobalBufferReadWriteData2Test)
{
    ...
    Buffer *buf = 
        RuntimeTestFixture::_bufMgr->createGlobalBuffer(device, CAL_FORMAT_INT_2, 78, 0);
...
TEST_F(GlobalBufferTests, GlobalBufferReadWriteData3Test)
{
    ...
    Buffer *buf1D = 
        RuntimeTestFixture::_bufMgr->createGlobalBuffer(device, CAL_FORMAT_DOUBLE_2, width, height);
